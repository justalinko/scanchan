#!/bin/bash
SCANCHAN=$(pwd)
SCANCHAN_WORK_DIR=$SCANCHAN
SCANCHAN_OUTPUT=$SCANCHAN/output/

save_output(){
	
	if [[ -f "$SCANCHAN_OUTPUT/.shutdown57" ]]; then
		mkdir $SCANCHAN_OUTPUT
		touch $SCANCHAN_OUTPUT/.shutdown57
		chmod 777 -R $SCANCHAN_OUTPUT
	fi
	nama="$SCANCHAN_OUTPUT/vulnsqli-"$(date +%d%m)
	echo ${1} >> $nama.txt
}
...
save_dork()
{
	...
	nama="$SCANCHAN_OUTPUT/list-dork-"$(date +%d%m)
	echo ${1} >> $nama.txt
}
startdir="../output/"
filext='txt'
menutitle="Select list"
